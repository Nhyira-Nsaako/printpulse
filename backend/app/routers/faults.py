...
async def fault_stats(
    ...
    This endpoint is public so the dashboard can retrieve
    statistics without requiring authentication.
    """
    ...
    return FaultStats(
        ...
        normal_count=await count(
            FaultEvent.fault_class == FaultClass.NORMAL
        ),

        mechanical_fault_count=await count(
            FaultEvent.fault_class == FaultClass.MECHANICAL_FAULT
        ),

        thermal_anomaly_count=await count(
            FaultEvent.fault_class == FaultClass.THERMAL_ANOMALY
        ),

        unacknowledged_faults=await count(
            and_(
                FaultEvent.fault_class != FaultClass.NORMAL,
                FaultEvent.acknowledged == False,
            )
        ),
...
async def get_fault(
    ...
    """Get a single fault event by ID."""
